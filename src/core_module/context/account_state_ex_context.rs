...
pub async fn get_tx_after_accounts_state(
    ...
    let mut turn_off_diff_accounts_state =
        get_turn_off_diff_accounts_state(provider.clone(), tx_hash).await;
    let mut turn_on_diff_post_accounts_state =
        get_turn_on_diff_post_accounts_state(provider, tx_hash).await;
        ...
        ethers::types::GethTrace::Known(ref geth_tracer_frame) => match geth_tracer_frame {
            ethers::types::GethTraceFrame::PreStateTracer(pre_state_frame) => match pre_state_frame
            {
                PreStateFrame::Default(default_mode) => {
                    ...
                    tx_account_state_ex = insert_tx_account_state_ex(
                        tx_account_state_ex,
                        true_off_pre_state,
                        is_diff,
                    );
